...
function NavBar({ setKeyword, setLocation, setViewPort, viewPort}) {
  ...
  return (
    <div className="flex flex-col justify-start items-center bg-[#191a1a] gap-6 p-4">
      <input
            ...
            ${
              ...
              onClick={() => {
                if (item?.state.toLowerCase() !== selected.toLowerCase()) {
                  ...
                  {item?.state === "USA" ? 
                  (setViewPort({...viewPort, latitude: item.latitude, longitude: item.longitude, zoom: 4})) : 
                  (setViewPort({...viewPort, latitude: item.latitude, longitude: item.longitude, zoom: 6})) }
